...
bool ZBuffer::FillReflectionBuffer(int x, int y, float depth, Color color, const Vecf3 & clipCoord)
{
	...
	Vecf3 testPoint = Pipeline<SurfaceDirectionalLighting>::pointOnWaterPlane;
	Vecf3 testVec = Pipeline<SurfaceDirectionalLighting>::perpendicularVectorToWaterPlane;
	...
	if (planeCheck < 0) {*/
		...
		if (depth < depthAtPoint) {
			depthAtPoint = depth;
			//reflectionBuffer[int(y*ratioValY)*reflectionBufferWidth + (reflectionBufferWidth - int(x*ratioValX))] = color;
