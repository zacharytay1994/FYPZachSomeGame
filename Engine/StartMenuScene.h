...
public:
	StartMenuScene(Graphics& gfx, std::stack<std::unique_ptr<MenuSceneParent>>& stackIn, std::vector<std::unique_ptr<Scene>>& scenes,
		const std::shared_ptr<FontList>& fontList)
		:
		MenuSceneParent("Start Menu", "startmenutest1.bmp", gfx, stackIn, scenes, Vecf3(255.0f, 255.0f, 255.0f), fontList)
	{
		// buttons
	...
	virtual void Update(Keyboard& kbd, Mouse& mouse, float dt) override {
		while (!mouse.IsEmpty()) {
			...
			if (OnClick(e)) {
				if (eventID == 1) {
					menuScenes.push(std::make_unique<MenuNaviTestScene>(gfx, menuScenes, scenes, fontList));
				}
				else if (eventID == 3) {
					testState = true;
				}
