...
public:
	EntityOne(const float& size, const Veci3& loc, const float& worldSize, const int& gridSize)
		:
		Entity(size, loc, worldSize, gridSize)
	{
		SetVelocity({ 1.0f, 0.0f, -1.0f });
	}
	EntityOne(const float& size, const Veci2& loc, const float& worldSize, const int& gridSize)
		:
		Entity(size, loc, worldSize, gridSize)
	{
		SetVelocity({ 1.0f, 0.0f, -1.0f });
	}
	EntityOne(const float& size, const Veci2& loc, const float& heightDisplaced, const float& worldSize, const int& gridSize)
		:
